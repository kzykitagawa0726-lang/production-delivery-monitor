...
def _write_supplier_capacity_forecast_sheet(ws: Worksheet, data: ReportData) -> None:
    ...
    ws.cell(
        ...
        value=(
            ...
            "\n"
            "⚠ 日程には標準LT(実績日数ではない)を使っているため、多くの受注がたまたま"
            "同じ週に集中して見える場合があります(特に現在停滞中の工程は、実際にはバラバラに"
            "動くはずが、同じ起点・同じ標準LTで計算するため同じ週に収束しやすい、という"
            "このモデル特有のクセです)。比率が非常に大きい(10倍超など)週は「その週に文字通り"
            "その金額が動く」という精密な予測ではなく、「その仕入先まわりの受注が今、多く"
            "滞留している/これから重なりそうだ」という方向性のシグナルとして捉えてください。"
            "内訳シート(仕入先週別予測内訳)でどの受注が積み上がっているかを確認するのが実務的です。"
        ),
    )
    ws.cell(row=note_row, column=1).font = Font(italic=True, color="FF9C0006")
...
def _write_capacity_forecast_note(ws: Worksheet) -> None:
    ...
    ws.cell(
        ...
        value=(
            ...
            "充足率を出せます。"
            "⚠ 現在同じ工程で停滞している受注は、同じ起点・同じ標準LTで計算するため同じ週に"
            "集中して見えることがあります(実際は個々にバラバラに動くはずですが、このモデルでは"
            "区別できません)。特定の週が突出して多い場合は、実務上バラけると考えて差し支えありません。"
        ),
    )
    ws.cell(row=note_row, column=1).font = Font(italic=True, color="FF9C0006")
